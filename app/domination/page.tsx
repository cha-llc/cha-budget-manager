    ...
    <Layout activeTab="domination">
      <div style={{ maxWidth: '1200px' }}>
        ...
        {activeTab === 'Checklist' && (
          <div>
            {/* Add new item */}
            <div style={{ ...card, marginBottom: '1.5rem', borderColor: 'rgba(201,168,76,0.4)' }}>
              ...
              <div style={{ display: 'grid', gridTemplateColumns: '1fr auto auto', gap: '0.75rem', alignItems: 'flex-end' }}>
                ...
                <button className="btn-primary" onClick={addItem} disabled={addingItem || !newItem.trim()}
            ...
            {loading ? (
              <div style={{ ...card, textAlign: 'center', padding: '2rem' }}><p style={{ color: 'rgba(255,255,255,0.4)', margin: 0 }}>Loading checklist...</p></div>
            ) : (
        ...
        {activeTab === 'Milestones' && (
          <div>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: '0.75rem' }}>
              {milestones.map(m => (
                <div key={m.id} className="card-hover" onClick={() => cycleMilestone(m)}
                  style={{ ...card, cursor: 'pointer', borderLeft: `3px solid ${milestoneStatusColor(m.status)}`, background: m.status === 'completed' ? 'rgba(42,157,143,0.07)' : m.status === 'building' ? 'rgba(201,168,76,0.05)' : 'rgba(255,255,255,0.03)', transition: 'all 0.2s ease' }}>
                  <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: '0.5rem' }}>
        ...
        {activeTab === 'Quarterly Targets' && (
          <div>
            <div style={{ ...card, marginBottom: '1.5rem', background: 'rgba(42,157,143,0.06)', borderColor: 'rgba(42,157,143,0.3)', textAlign: 'center', padding: '2rem' }}>
...
function MilestoneAddForm({ onAdd }: { onAdd: (label: string, icon: string) => void }) {
  ...
  return (
    <div style={{ display: 'grid', gridTemplateColumns: '1fr auto auto', gap: '0.75rem', alignItems: 'flex-end' }}>
      ...
      <button className="btn-primary" onClick={() => { if (label.trim()) { onAdd(label, icon); setLabel(''); } }}
